...
BOOL slavesListRemoveByPath(const char *path, unsigned int pathSize)
{
	if (isListEmpty(slavesListHead))
	{
		return FALSE;
	...
	slavesList *prev = NULL;
	slavesList *curr = slavesListHead;
	while (curr != NULL)
	{
		if (strncmp(curr->path, path, pathSize) == 0)
		{
			if (prev == NULL)
				slavesListHead = curr->next;
			else
				prev->next = curr->next;

			if (curr == slavesListTail)
				slavesListTail = prev;

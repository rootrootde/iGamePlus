...
#define JOY_REPEAT_DELAY 10 /* ticks before auto-repeat starts (~200ms) */
#define JOY_REPEAT_RATE 2   /* ticks between repeats (~40ms) */
	int joy_repeat_count = 0;
#endif
	...
	while (running)
	{
		...
		switch (eventId)
		{
			...
			case MENU_TOGGLEFAVORITE:
				game_toggle_favourite();
				break;
		...
		if (LowLevelBase)
		{
			...
			if (new != old)
			{
				old = new;
				joystick_input(new);
				joy_repeat_count = JOY_REPEAT_DELAY;
			}
			else if (new & (JPF_JOY_UP | JPF_JOY_DOWN | JPF_JOY_LEFT | JPF_JOY_RIGHT))
			{
				if (joy_repeat_count > 0)
					joy_repeat_count--;
				else
				{
					joystick_direction_repeat(new);
					joy_repeat_count = JOY_REPEAT_RATE;
				}
			}
			else
			{
		...
		if (running && signals && !screenshot_is_pending()) Wait(signals);
	}
